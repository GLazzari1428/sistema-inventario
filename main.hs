...
tratarResultado :: UTCTime
                ...
                -> IO Resultado
tratarResultado time acao falhaMsg inv logs (Left erro) = do
    putStrLn erro
    let logFalha = LogEntry time acao falhaMsg (Falha erro)
    salvarLogEntry logFalha
    return (inv, logs ++ [logFalha])

-- SUCESSO: exibe msg, salva inventário e loga Sucesso
tratarResultado _ _ _ _ logs (Right (novoInv, logSucesso)) = do
    putStrLn $ "Sucesso: " ++ detalhes logSucesso
    salvarInventario novoInv
    salvarLogEntry logSucesso
    return (novoInv, logs ++ [logSucesso])
      ...
      ["add", id', nome', qtdStr, cat'] -> do
        case readMaybe qtdStr of
          ...
          Just qtd -> do
            ...
            tratarResultado time Add ("Tentativa de adicionar: " ++ id') inv logs (addItem time item inv)
      ...
      ["remove", id', qtdStr] -> do
        case readMaybe qtdStr of
          Nothing -> do
            ...
            return (inv, novoLogs)
          Just qtd -> tratarResultado time Remove ("Tentativa de remover: " ++ id') inv logs (removeItem time id' qtd inv)
      ...
      ["update", id', qtdStr] -> do
        case readMaybe qtdStr of
          Nothing -> do
            ...
            novoLogs <- logFalhaIO time comando "Quantidade nao numerica." logs
            return (inv, novoLogs)
          Just qtd -> tratarResultado time Update ("Tentativa de atualizar: " ++ id') inv logs (updateQty time id' qtd inv)
